...
def test_pick_words_to_remove():
    ...
    tokens = helpers.get_token_info_basic("Król Karol kupił Królowej Karolinie korale koloru koralowego")
    chosen = fill.pick_words_to_remove(tokens,3)
    for i in range(2):
        assert chosen[i] in tokens

def test_pick_words_to_remove_wont_double_words():
    tokens = helpers.get_token_info_basic("Test test1 Test2 test1 Test Test1")
    for i in range(3):
...
def test_replace_word_token(monkeypatch):
    ...
    page = "Test page"
    tokens = helpers.get_token_info_basic(page)
